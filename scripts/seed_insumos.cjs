...
const pool = new Pool({
  connectionString: 'postgresql://postgres:MerinoTrueba17@db.kcfvjorpgucdrbjbfdef.supabase.co:5432/postgres',
...
async function run() {
  ...
  try {
    ...
    await client.query(`
      DROP TRIGGER IF EXISTS trg_actualizar_precio_promedio ON compras;
    ...
    let inserted = 0;
    for (const [tipo, marca, presentacion, precio, ml] of rows) {
      const res = await client.query(
        `INSERT INTO insumos (tipo_insumo, marca, presentacion, precio_promedio, ml_gr_pieza)
         VALUES ($1,$2,$3,$4,$5) ON CONFLICT (marca, presentacion) DO NOTHING`,
        [tipo, marca, presentacion, precio, ml]
      );
      inserted += res.rowCount;
    }
    console.log(`✓ Insertados ${inserted} insumos`);

    // ── RLS ──────────────────────────────────────────────────
    await client.query(`ALTER TABLE insumos ENABLE ROW LEVEL SECURITY`);
    await client.query(`ALTER TABLE compras ENABLE ROW LEVEL SECURITY`);
    await client.query(`
      DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='insumos' AND policyname='auth_full_access_insumos') THEN
          CREATE POLICY auth_full_access_insumos ON insumos FOR ALL TO authenticated USING (true) WITH CHECK (true);
        END IF;
      END $$
    `);
    await client.query(`
      DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='compras' AND policyname='auth_full_access_compras') THEN
          CREATE POLICY auth_full_access_compras ON compras FOR ALL TO authenticated USING (true) WITH CHECK (true);
        END IF;
      END $$
    `);
